...
{
	//Must contain 4 characters or more
	var WeakPass = /(?=.{4,}).*/; 
	//Must contain lower case letters and at least one digit.
	var MediumPass = /^(?=\S*?[a-z])(?=\S*?[0-9])\S{4,}$/; 
	//Must contain at least one upper case letter, one lower case letter and one digit.
	var StrongPass = /^(?=\S*?[A-Z])(?=\S*?[a-z])(?=\S*?[0-9])\S{4,}$/; 
	//Must contain at least one upper case letter, one lower case letter and one digit.
	var VryStrongPass = /^(?=\S*?[A-Z])(?=\S*?[a-z])(?=\S*?[0-9])(?=\S*?[^\w\*])\S{4,}$/; 
	...
	$(password1).on('keyup', function(e) {
		...
		{
			passwordsInfo.removeClass().addClass('weakpass').html("Very Weak! (Must be 4 or more chars)");
		}
	});
